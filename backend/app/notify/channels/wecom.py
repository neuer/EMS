"""企业微信群机器人渠道：POST text 消息到 webhook_url。

config：webhook_url（必填）、secret（敏感，可选）。
"""
...
class WecomAdapter:
    ...
    async def send(
        ...
        raise_for_business_error(body, channel="企业微信")  # 200+errcode!=0 视为失败
    ...
    async def test(self, config: dict[str, Any]) -> None:
        ...
        raise_for_business_error(body, channel="企业微信")
