...
M_EMS_RELOGIN = "ems_relogin"  # token/心跳失效(2/106)触发强制重登重订阅（红线 #4，审查 M1）
M_EMS_HEARTBEAT = "ems_heartbeat_failed"  # 心跳调用失败（区别于整体离线，审查 M8）
M_CONFIG_SYNC = "config_sync_failed"  # 定时配置同步失败
M_BACKFILL_WATCH = "backfill_watch_failed"  # 回补巡检自身失败
M_CAGG_REFRESH = "cagg_refresh_failed"  # 5min 连续聚合刷新失败（降采样层可能缺数，审查 M8）
M_NOTIFY_DIGEST = "notify_digest_failed"  # 摘要刷新中 Redis 故障（审查 M8）
